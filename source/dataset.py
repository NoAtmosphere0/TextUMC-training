...
HOME_DIR = BASE_DIR + "/drive/MyDrive/thesis/"


DATASET2PATH = {
    "LIAR_RAW": os.path.join(HOME_DIR + "datasets/LIAR-RAW/"),
    "RAWFC": os.path.join(HOME_DIR + "datasets/RAWFC/"),
    "RAWFC_step2": os.path.join(HOME_DIR + "datasets/RAWFC_step2/"),
    "LIAR-RAW_step2": os.path.join(HOME_DIR + "datasets/LIAR-RAW_step2/"),
}
...
def get_raw_datasets(dataset, dataset_dir=None):
    if dataset == "LIAR_RAW":
        ...
        train_dataset_raw, dev_dataset_raw, test_dataset_raw = [
            read_json(_p)
            for _p in [train_dataset_path, dev_dataset_path, test_dataset_path]
        ]
    elif dataset == "RAWFC":
        dataset_dir = dataset_dir or DATASET2PATH[dataset]
        ...
        train_dataset_path = os.path.join(
            dataset_dir, "train_10_evidence_details.json"
        )  # if top-k == 10
...
class NewsDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        (
            ...
            self.report_domains,
            self.report_sents,
            self.report_sents_labels,
            self.report_sents_belong,
            self.num_sentences_per_report,
        ) = self.load_raw(news_dataset)
    ...
    def __getitem__(self, index):
        ...
        claim_tokenized = self.tokenizer(
            self.claim[index],
            return_tensors="pt",
            padding=True,
            truncation=True,
        ...
        claim_input_ids, claim_mask_ids = (
            claim_tokenized["input_ids"].squeeze(),
            claim_tokenized["attention_mask"].squeeze(),
        )
        claim_label_id = torch.tensor(
            self.label_dict[self.label[index]], dtype=torch.long
        )

        sent_tokenized = self.tokenizer(
            self.report_sents[index],
            return_tensors="pt",
            padding=True,
            truncation=True,
            max_length=self.max_seq_length,
        )
        sent_input_ids, sent_mask_ids = (
            sent_tokenized["input_ids"],
            sent_tokenized["attention_mask"],
        )
        sent_label_ids = torch.tensor(self.report_sents_labels[index], dtype=torch.long)

        decoder_input_tokenized = self.tokenizer(
            self.explain[index],
            return_tensors="pt",
            padding=True,
            truncation=True,
        ...
        decoder_input_ids, decoder_mask_ids = (
            decoder_input_tokenized["input_ids"].squeeze(),
            decoder_input_tokenized["attention_mask"].squeeze(),
        )

        num_sentences_per_report = torch.tensor(
            self.num_sentences_per_report[index], dtype=torch.long
        )
            ...
            if _idx_t == 3 or _idx_t == 4:  # sent_input_ids, sent_mask_ids
                ...
                for _tensor in _tensors:
                    ...
                    for idx, _ in enumerate(list(_tensor)):
                        ...
                        _pad_tensor = torch.tensor(
                            [padding_value] * _pad_shape,
                            device=_tensor[idx].device,
                            dtype=_tensor[idx].dtype,
                        )
                        _new_inner_tensor = torch.cat(
                            [_tensor[idx], _pad_tensor], dim=0
                        )
                ...
                return_list.append(
                    torch.nn.utils.rnn.pad_sequence(
                        _new_tensors, batch_first=True, padding_value=padding_value
                    ),
                )
            else:
                if _tensors[0].dim() >= 1:
...
class Stage2DatasetForLLM(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.true_evidences, self.false_evidences = self.format(
            self.true_sents, self.false_sents
        )
    ...
    def format(self, true_sents, false_sents):
        ...
        for idx in range(len(true_sents)):
            ...
            true_evidences_list = [
                sent
                for i, sent in enumerate(true_sents[idx])
                if i < self.num_evidence_sentences
            ]
            sample_true_evidences = "\n".join(true_evidences_list)
            false_evidences_list = [
                sent
                for i, sent in enumerate(false_sents[idx])
                if i < self.num_evidence_sentences
            ]
    ...
    def __getitem__(self, index):
        ...
        return (
            self.claims[index],
            self.true_evidences[index],
            self.false_evidences[index],
        )
...
class Stage2PredictionDatasetForRoBERTa(Dataset):
        ...
        **kwargs
        ...
        self.explanations = load_json(
            os.path.join(
                ...
                + "/"
                + dataset_type
                + "_label_oriented_explanation.json"
            )
        )
    ...
    def load_raw(self, news_dataset):
        ...
        if self.nums_label == 6:
            assert (
                "LIAR" in self.dataset_name
            ), "Invalid dataset_name for six labels: {}".format(self.dataset_name)
    ...
    def format(self):
        ...
        for i, claim in tqdm(enumerate(self.claims)):
            ...
            input = (
                self.prompt.replace("[CLAIM]", claim)
                .replace("[TRUE_EXPLANATION]", "[" + true_exp + "]")
                .replace("[FALSE_EXPLANATION]", "[" + false_exp + "]")
            )
    ...
    def __getitem__(self, index):
        ...
        input_id, _ = self.tokenizer(
            sent, return_tensors="pt", truncation=True, max_length=self.max_seq_length
        ).values()
