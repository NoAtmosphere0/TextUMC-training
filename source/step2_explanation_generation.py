...
DATASET_DIR = BASE_DIR + "/drive/MyDrive/thesis/datasets"
...
def get_reference_explanation_llama2(args, dataset, dataset_type):
    ...
    results = []
    count = 0
    prompts = get_prompts_for_explanation_generation(
        dataset.claims, dataset.true_evidences, dataset.false_evidences, tokenizer
    )
    ...
    for _idx, prompt in enumerate(
        tqdm(prompts, desc=dataset_type + "_llama2 explanation generation")
    ):
        ...
        if count == 100:
            count = 0
            save_json(
                results, save_dir + dataset_type + "_label_oriented_explanation.json"
            )
...
def get_reference_explanation_gpt(args, dataset, dataset_type):
    ...
    prompts = get_prompts_for_explanation_generation(
        dataset.claims, dataset.true_evidences, dataset.false_evidences
    )
    ...
    for _idx, prompt in enumerate(
        tqdm(prompts, desc=dataset_type + "_gpt_turbo explanation generation")
    ):
...
def get_prompts_for_explanation_generation(
    claims, true_sentences, false_sentences, tokenizer=None
):
    prompts = []
    if tokenizer is not None:  # for llama2
        ...
        for i, claim in tqdm(enumerate(claims), desc="Generate context"):
            true_instruction = (
                EXPLANATION_USER_PROMPT_v0.replace("[CLAIM]", claim)
                .replace("[LABEL]", "True")
...
def main():
    ...
    train_dataset = Stage2DatasetForLLM(
        args.dataset_name,
        train_raw_dataset,
        num_evidence_sentences=args.num_evidence_sentences,
    )  # Stage2Dataset
    eval_dataset = Stage2DatasetForLLM(
        args.dataset_name,
        eval_raw_dataset,
        num_evidence_sentences=args.num_evidence_sentences,
    )
    test_dataset = Stage2DatasetForLLM(
        args.dataset_name,
        test_raw_dataset,
        num_evidence_sentences=args.num_evidence_sentences,
    )
