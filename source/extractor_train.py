...
def train(args, train_dataset, model, tokenizer, eval_dataset=None, eval_fn=None):
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=train_dataset.data_collate_fn,
    )
    ...
    warmup_steps = (
        ...
        else int(args.warmup_proportion * t_total)
    )
    optimizer = AdamW(
        model.parameters(),
    ...
    for _idx_epoch, _ in enumerate(train_iterator):
        epoch_iterator = tqdm(
            train_dataloader,
            desc="Iteration-{}({})".format(
                _idx_epoch, args.gradient_accumulation_steps
            ),
            disable=args.local_rank not in [-1, 0],
        )
        for step, batch in enumerate(epoch_iterator):
            model.train()
            batch_tensor, batch_text = (
                tuple(t.to(args.device) for t in batch[0]),
                batch[1],
            )

            stage1_outputs = model(
                batch_tensor[0],
                batch_tensor[1],
                batch_tensor[2],
                batch_tensor[3],
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    with open(
                        os.path.join(args.output_dir, "loss_record.txt"), "a"
                    ) as wf:
                        wf.write(
                            "ep:"
                            + str(_idx_epoch)
                        ...
                        wf.write(
                            f"Loss: {loss:.4f}\t KL-Loss: {kl_loss:.4f}\t CLS_loss: {cls_loss:.4f}"
                        )
                        wf.write("\n")
                ...
                if (
                    ...
                    results = eval_fn(
                        args, eval_dataset, model, tokenizer, global_step=global_step
                    )
                    for key, value in results.items():
                        tb_writer.add_scalar("eval_{}".format(key), value, global_step)
                    with open(
                        os.path.join(args.output_dir, "eval_results.txt"), "a"
                    ) as wf:
                        wf.write(
                            "ep"
                            + str(_idx_epoch)
                    ...
                    if results["f1"] > metric_best:
                        save_model_with_default_name(
                            args.output_dir, model, tokenizer, args
                        )
        ...
        if args.save_checkpoint:
            save_model_with_default_name(
                args.output_dir + "/checkpoints/" + str(_idx_epoch),
                model,
                tokenizer,
                args,
            )
...
def evaluation(
    ...
    eval_dataloader = DataLoader(
        ...
        shuffle=False,
        collate_fn=eval_dataset.data_collate_fn,
    )
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for _i in range(len(batch[1])):
            ...
            sample_sorted_predict_sent_scores, sample_sorted_predict_sent_idx = (
                torch.sort(sample_predict_sent_scores, descending=True)
            )
            # select top-k evidence sentences
            selected_num = min(args.top_k, len(sample_sorted_predict_sent_idx))

            sample_sorted_predict_sent_idx = (
                sample_sorted_predict_sent_idx[:selected_num].detach().cpu().numpy()
            )
            sample_predicted = np.zeros(len(sample_predict_sent_scores))
            sample_predicted[sample_sorted_predict_sent_idx] = 1

            sample_pre = precision_score(
                batch_sents_labels[_i], sample_predicted, average="binary", pos_label=1
            )
            ...
            if args.get_evidences:
                sent_false_scores, sent_false_idx = torch.sort(
                    false_scores[_i], descending=True
                )
                false_selected_num = min(args.top_k, len(false_scores))

                sent_false_idx = (
                    sent_false_idx[:false_selected_num].detach().cpu().numpy()
                )
                sent_false_scores = (
                    sent_false_scores[:false_selected_num].detach().cpu().numpy()
                )
                sent_false_scores = [
                    round(score, 3) for score in (10000 * sent_false_scores).tolist()
                ]
                ...
                sent_true_scores, sent_true_idx = torch.sort(
                    true_scores[_i], descending=True
                )
                true_selected_num = min(args.top_k, len(true_scores))

                sent_true_idx = sent_true_idx[:true_selected_num].detach().cpu().numpy()
                sent_true_scores = (
                    sent_true_scores[:true_selected_num].detach().cpu().numpy()
                )
                sent_true_scores = [
                    round(score, 3) for score in (10000 * sent_true_scores).tolist()
                ]
                ...
                zipped_true_evidence_details = zip(
                    true_evidence_text,
                    sent_true_scores,
                    sent_true_labels.tolist(),
                    sent_true_idx.tolist(),
                )
                true_evidence_details = [
                    [a, b, c, d] for a, b, c, d in zipped_true_evidence_details
                ]

                zipped_false_evidence_details = zip(
                    false_evidence_text,
                    sent_false_scores,
                    sent_false_labels.tolist(),
                    sent_false_idx.tolist(),
                )
                false_evidence_details = [
                    [a, b, c, d] for a, b, c, d in zipped_false_evidence_details
                ]

                tmp_evidence_dict[batch[1][_i]["event_id"]]["claim"] = batch[1][_i][
                    "claim"
                ]
                tmp_evidence_dict[batch[1][_i]["event_id"]]["label"] = int(
                    batch[0][2][_i]
                )

                tmp_evidence_dict[batch[1][_i]["event_id"]]["predicted_label"] = int(
                    batch_predictions[_i].detach().cpu().numpy()
                )
                tmp_evidence_dict[batch[1][_i]["event_id"]]["predicted_score"] = [
                    "{:.3f}".format(e)
                    for e in list(predict_scores[_i].detach().cpu().numpy())
                ]
                tmp_evidence_dict[batch[1][_i]["event_id"]][
                    "logsoftmax_predicted_score"
                ] = [
                    "{:.3f}".format(e)
                    for e in list(
                        torch.softmax(predict_scores[_i], dim=0).detach().cpu().numpy()
                    )
                ]
                tmp_evidence_dict[batch[1][_i]["event_id"]]["explain"] = batch[1][_i][
                    "explain"
                ]
                tmp_evidence_dict[batch[1][_i]["event_id"]][
                    "true_details"
                ...
                tmp_evidence_dict[batch[1][_i]["event_id"]]["num_overlap"] = [
                    len(sent_true_idx),
                    len(sent_false_idx),
                    len(set(sent_true_idx) | set(sent_false_idx)),
                ]
    ...
    if args.get_evidences:
        ...
        with open(
            os.path.join(
                args.output_dir,
                dataset_type + "_" + str(args.top_k) + "_evidence_details.json",
            ),
            "w",
        ) as f:
...
def main():
    ...
    train_raw_dataset, eval_raw_dataset, test_raw_dataset = get_raw_datasets(
        args.dataset_name
    )
    ...
    eval_dataset = NewsDataset(
        args.dataset_name, eval_raw_dataset, tokenizer, args.max_seq_length
    )
    test_dataset = NewsDataset(
        args.dataset_name, test_raw_dataset, tokenizer, args.max_seq_length
    )
    ...
    if args.do_prediction:  # view the performance of the temp detection
        results = evaluation(
            args, test_dataset, model, tokenizer, global_step=None, mode="test"
        )
        with open(
            os.path.join(args.output_dir, str(args.top_k) + "_test_result.txt"), "w"
        ) as wf:
    ...
    if (
        args.get_evidences
    ):  # used to get the extracted evidences, call it after the training.
        for dataset, dtype in zip(
            [eval_dataset, test_dataset, train_dataset], ["eval", "test", "train"]
        ):
            results = evaluation(
                args,
                dataset,
