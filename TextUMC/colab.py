...
# Key Components:
    ...
    - Contrastive Learning:
        ...
        - Supervised Contrastive Loss: Uses label information obtained by the clustering step to further guide training, forcing points within the same cluster to be close.
    - Training Strategies:
...
def train_loop(
    ...
    """Performs a single training epoch and displays iteration progress."""
    model.train()  # Set model to training mode
    total_loss, unsup_loss_total, sup_loss_total = 0, 0, 0
    ...
    for batch_idx, batch_texts in enumerate(progress_bar):
        ...
        _, pseudo_labels, _ = cluster_and_select_samples(
            embeddings,
            num_clusters,
            0.5,
            None,
            model.device,  # current_centroids removed from loop scope
        )

        # Calculate supervised contrastive loss
        sup_loss = supervised_contrastive_loss(embeddings, pseudo_labels, temp=temp_2)
    ...
    avg_sup_loss = sup_loss_total / num_batches

    print(
        f"Epoch [{epoch+1}/{int(epochs)}] Training Loss: {avg_loss:>7f} (Unsupervised: {avg_unsup_loss:>7f}, Supervised: {avg_sup_loss:>7f})"
    )
    return avg_loss, avg_unsup_loss, avg_sup_loss


def eval_loop(dataloader, model, num_clusters, temp_1, temp_2, epoch, epochs):
    ...
    with torch.no_grad():  # Disable gradients during validation
        for batch_idx, batch_texts in enumerate(progress_bar):
            ...
            # Clustering and pseudo-label generation
            _, pseudo_labels, _ = cluster_and_select_samples(
                embeddings, num_clusters, 0.5, None, model.device
            )
    ...
    print(
        f"Epoch [{epoch+1}/{int(epochs)}] Validation Loss: {avg_loss:>7f} (Unsupervised: {avg_unsup_loss:>7f}, Supervised: {avg_sup_loss:>7f})"
    )
    return avg_loss, avg_unsup_loss, avg_sup_loss
...
def umc_train(
    ...
    """Train model on evidence texts using combined UMC loss with epoch and batch structure and validation."""
    ...
    try:
        for epoch in progress_bar_epoch:
            epoch_loss, epoch_unsup_loss, epoch_sup_loss = train_loop(
                train_dataloader,
                model,
                model.optimizer,
                num_clusters,
            ...
            val_loss, val_unsup_loss, val_sup_loss = eval_loop(
                val_dataloader, model, num_clusters, temp_1, temp_2, epoch, num_epochs
            )

            metrics["epoch_losses"].append(epoch_loss)
            metrics["unsup_losses"].append(epoch_unsup_loss)
            metrics["sup_losses"].append(epoch_sup_loss)
            metrics["val_losses"].append(val_loss)
            metrics["val_unsup_losses"].append(val_unsup_loss)
            metrics["val_sup_losses"].append(val_sup_loss)
...
def _save_embeddings(model, claims):
    ...
    for claim in claims:
        ...
        embedding = model(claim_text).detach().cpu().numpy()
...
def print_evidence_clusters(claim: Claim, cluster_labels: np.ndarray):
    ...
    print(f"Evidence Clusters for Claim: {claim.claim_id}")
    ...
    for cluster_id in range(max(cluster_labels) + 1):
        ...
        for idx, evidence in enumerate(cluster_evidences):
            ...
            print(f"Cluster {cluster_id}" if idx == 0 else "", content)
...
def main() -> None:
    ...
    batch_size = 32
    epochs = 10
    save_after_n_epochs = None  # Set to > 0 to save model every n epochs
    ...
    try:
        ...
        model_config = ModelConfig(learning_rate=learning_rate, batch_size=batch_size)
        model = TextUMC(config=model_config).to(device)
        print(f"Model initialized on {device}")

        if args.model_checkpoint:
            model.load_model(args.model_checkpoint)
            print(f"Model loaded from checkpoint: {args.model_checkpoint}")
