...
def train_loop(
    ...
    total_iterations,
    epochs,
    current_epoch=0,  # New parameter for training from a checkpoint
):
    ...
    progress_bar = tqdm(
        dataloader,
        desc=f"Epoch [{current_epoch + epoch + 1}/{int(epochs)}]",  # Updated description with checkpoint epoch value
        unit="batch",
        leave=False,
    )
...
def main() -> None:
    ...
    try:
        ...
        print(
            f"Current epoch {current_epoch}. Metrics from model checkpoint: {metrics}"
        )
        ...
        if args.eval_only:
            ...
            torch.save(
                {
                    "epoch": args.num_epochs + current_epoch,
